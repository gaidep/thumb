...
if(! defined('NOT_FOUND_IMAGE') )		define ('NOT_FOUND_IMAGE', 'error.jpg');								// Image to serve if any 404 occurs 
if(! defined('ERROR_IMAGE') )			define ('ERROR_IMAGE', 'error.jpg');									// Image to serve if an error occurs instead of showing error message 
...
if(! defined('DEFAULT_Q') )				define ('DEFAULT_Q', 85);									// Default image quality. Allows overrid in timthumb-config.php
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if ($new_width == 0 && $new_height == 0) {
    		...
    		$watermark_path = $this->cacheDirectory . '/logo.png';
    		...
    		$fileType = strtolower($sData['mime']);
    		...
    		switch($fileType)
    		{
    			...
    			case	'image/png': 	$image_id = imagecreatefrompng($localImage);imagealphablending($image_id, true);
    		imagesavealpha($image_id, true);	break;
    			default:				$image_id = imagecreatefromjpeg($localImage);	break;
    		}
    		if(!$is_gif){
    			...
    			*/
    			...
    			switch($fileType)
    			{
    				case	'image/png': 	@imagepng ($image_id, $localImage); 		break;
    				default:				@imagejpeg($image_id, $localImage, 90); 		break;
    			}       		 
	...
	protected function getURL($url, $tempfile){
		...
		if(function_exists('curl_init')){
            ...
            curl_setopt ($curl, CURLOPT_REFERER, $iHost);
